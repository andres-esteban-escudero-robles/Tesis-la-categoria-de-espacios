...
\begin{proof}
  ...
  entonces $1$ es el subobjeto maximal, por lo que $\varnothing \to 1\times 1$ es el complemento que buscamos.
\end{proof}

\begin{corollary}
  Todo subespacio de 1 es discreto.
\end{corollary}

\begin{theorem}
  Sea $A\in Sub(1)$, entonces $A$ tiene complemento.
\end{theorem}
\begin{proof}
  Procedemos por casos. Si $A \cong \varnothing$ entonces $1$ es su complemento, en otro caso,
  aplicando \ref{ax:gds6} tenemos que existe $a:1\to A$. En este caso, proponemos al pseudocomplemento
  $\lnot A$ como el complemento de $A$. En automático tenemos que $A\cap \lnot A = \varnothing$,
  para $A\cup \lnot A = 1$ vemos que
  \begin{equation*}
    \begin{tikzcd}
      1\ar{r}{a} \ar[swap]{rd}{id} & A\ar{r}{i_A} & A+\lnot A \ar{r} & A\cup\lnot A\ar[tail]{r}{m} &1 \\
        & 1\ar{urrr}
    \end{tikzcd}
  \end{equation*}
  El diagrama conmuta pues acaba en el terminal y como la composición es igual a la identidad
  en $1$, que es un epimorfismo, entonces $m$ también es epi. Por tanto, $A\cup \lnot A \cong 1$.
\end{proof}
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{tikzcd}
      1\ar{r}{i_1}\ar[swap]{rdd}{ai_1}\ar{rd}{o} & 1+N\ar{d}{[o,s]} & N\ar[swap]{l}{i_N}\ar{ldd}{fs}\ar{ld}{s} & {} & 1\ar[swap]{r}{i_1}\ar{rdd}{ai_1}\ar{rd}{i_1} & 1+N\ar{d}{[i_1,i_Af]} & N\ar[swap]{l}{i_N}\ar{ldd}{fs}\ar{ld}{i_Af} \\
        & N\ar{d}{f} & {} & {} & {} & 1+A\ar{d}{a} & {} \\
        & A & {} & {} & {} & A & {}
    \end{tikzcd}
  \end{equation*}
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{tikzcd}
      1\ar{r}{o}\ar[swap]{rd}{o'}\ar{rdd}{o} & N\ar{d}{f}\ar{r}{s} & N\ar{d}{f} \\
...
\begin{proof}
  ...
  $~:N\times N\to N$ tal que $~(a,0)=a$ y $~(a,n+1)=p(~(a,n))$ donde $p$ es el predecesor de \ref{prop:sucesormono}.
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  Que son complementados gracias a que ser complementado es estable bajo productos fibrados. Como $n~n=0$ para toda $n$,
  $\Delta$ es subobjeto de $N_1$ y por lo tanto $\Delta$ también tiene complemento en $Sub(N\times N)$.
\end{proof}

\begin{theorem}
  \ref{ax:gds6} y \ref{ax:gds7} inducen una pareja de funtores adjuntos entre $M$, la
...
\end{lemma}
\begin{proof}
  Seguiremos la demostración de Maclane \cite{maclane2012sheaves}. Sean $(A,s), (B,t)$ y $(C,u)$ coálgebras, entonces tenemos la siguiente cadena de isomorfismos naturales:
  \begin{equation*}
...
\begin{theorem}
  Si $\Omega$ es el clasificador de subobjetos en $\mathcal{E}$, entonces $\Gamma\Omega$ con 
