...
\begin{proof}
  ...
  $m$ induce $m\times m\colon S\times S \rightarrowtail A\times A$ y $(m\times m)^{-1}(\Delta_A)=\Delta_S$ ya que
  \begin{equation*}
    \begin{tikzcd}
...
\end{theorem}
\begin{proof}
  Sea $A$ un subespacio de $1$, con $m\colon A\to 1$. Sin perder generalidad, supongamos que $A$ no es isomorfo a $0$, entonces por \ref{ax:gds6} tenemos que existe $a\colon 1\to A$. 
...
\begin{proof}
  ...
  por definición de coproducto tenemos $i_1\colon 1\to 1+A$ e $i_A\colon A\to 1+A$ entonces:
  \begin{equation*}
    \begin{tikzcd}
...
\end{proposition}
\begin{proof}
  Primero construimos al morfismo ``predecesor'' $p\colon N\to N$ tal que $p(0)=0$ y $p(n+1)=n$ con el siguiente diagrama:
  \begin{equation*}
    \begin{tikzcd}
...
\begin{proof}
  ...
  $\dot{-}\colon N\times N\to N$ tal que $\dot{-}(a,0)=a$ y $\dot{-}(a,n+1)=p(\dot{-}(a,n))$ donde $p$ es el predecesor de \ref{prop:sucesormono}.
