...
\begin{proposition}[label=prop:terminaldiscreto]
  1 es un espacio discreto.\pdfcomment{El objeto terminal 1 es discreto.
  Esto para que el enunciado no empiece con un número. Además, recuerda poner al
  1 en modo matemático.}
\end{proposition}
\begin{proof}
...
\end{theorem}
\begin{proof}
  Sea $A$ un subespacio de $1$, \pdfcomment{Nota que ``con m:
  A -> 1'' no dice nada. Debería decir ``con m: A -> 1 mono''.}con $m\colon A\to 1$. Juntando nuestras hipótesis obtenemos al siguiente diagrama:
  \begin{equation*}
    \begin{tikzcd}
  ...
  en $1$, $m$ es un epimorfismo que se divide\pdfcomment{creo que lo usual es
  decir que se escinde} aparte de ser mono. Por tanto, $m$ es un isomorfismo y $A\cong 1$.
\end{proof}
...
\begin{proof}
  ...
  1+A$, entonces:
  \pdfcomment{entonces existe una única f: N -> A que hace conmutar al siguiente diagrama}
  \begin{equation*}
    \begin{tikzcd}
