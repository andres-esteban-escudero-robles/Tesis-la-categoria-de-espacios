...
\begin{proof}
  ...
  \pdfcomment{ya que el siguiente es un producto fibrado (Sólo poner el cuadrado})
  \begin{equation*}
    \begin{tikzcd}
...
\begin{theorem}
  ...
  \pdfcomment{Si A está en Sub(1) y tiene un punto, entonces A es isomorfo a 1.}
\end{theorem}
\begin{proof}
  Sea $A$ un subespacio de $1$, con $m\colon A\to 1$. Sin perder generalidad,
  supongamos que $A$ no es isomorfo a $0$, entonces por
  \ref{ax:gds6}\pdfcomment{No será necesario referir al axioma} tenemos
  que existe $a\colon 1\to A$. 
