...
Notamos que, en el caso de Espacios, todo subespacio de $1$ o es $0$ o tiene 
un punto, según el axioma~\ref{ax:gds6}, así que el teorema anterior nos dice que $\sub(1)=\{0,1\}$.
...
\begin{definition}[label=def:bivaluado]
  Un topos es bivaluado si su clasificador de subobjetos tiene exactamente dos puntos, verdadero
  y falso. En particular, un topos bivaluado no es trivial y, con el resultado anterior, Espacios es bivaluado.
\end{definition}
...
\begin{definition}
  ...
  cada objeto $A$ de $\mathcal{E}$, $\sub(A)$ es una álgebra de Boole, es decir, tercero excluido es válido internamente.
\end{definition}

Ser bivaluado sí implica tercero excluido para fórmulas sin variables libres, pero esto no basta
ni siquiera para que las leyes clásicas de cuantificadores se cumplan. Por ejemplo, en Espacios,
podemos demostrar para el espacio $D$ de infinitesimales tanto que $\forall x\in D, \neg\neg(x=0)$ como que $\neg(\forall x\in D, x=0)$,
por lo que Espacios es bivaluado, pero no es booleano. Podemos evidenciar que bivaluado no implica booleano, 
así como booleano no implica bivaluado, con ejemplos más sencillos y abundantes.

\begin{proposition}
  El topos $Conjuntos\times  Conjuntos$ es booleano y no bivaluado. 
\end{proposition}
\begin{proof}
  ...
  hay cuatro valores de verdad. Por otro lado, el coproducto se calcula como el coproducto de Conjuntos entrada a entrada,
...
\begin{proposition}
  ...
  $f\circ f = f$, el topos $Conjuntos^M$ es bivaluado y no booleano.
\end{proposition}
\begin{proof}
  Los objetos de $Conjuntos^M$ son pares $(X,h)$ donde $X$ es un conjunto y $h\colon X\to X$ es 
